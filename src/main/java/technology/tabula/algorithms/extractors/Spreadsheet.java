...
import technology.tabula.tables.TableWithRulingLines;
 ...
 * @author manuel
 */
public class Spreadsheet implements ExtractionAlgorithm {

    private static final float MAGIC_HEURISTIC_NUMBER = 0.65f;

    private static final Comparator<Point2D> POINT_COMPARATOR = (point1, point2) -> {
        float point1X = Utils.round(point1.getX(), 2);
        float point1Y = Utils.round(point1.getY(), 2);
        float point2X = Utils.round(point2.getX(), 2);
        float point2Y = Utils.round(point2.getY(), 2);
    ...
    public boolean isTabular(Page page) {
        ...
        List<? extends Table> tables = new Spreadsheet().extract(minimalRegion);
        if (tables.size() == 0) {
            return false;
        }
        ...
        tables = new Basic().extract(minimalRegion);
        if (tables.size() == 0) {
    ...
    static class PolygonVertex {
        ...
        public String toString() {
            return String.format("%s[point=%s,direction=%s]", getClass().getName(), point.toString(), direction.toString());
        }
